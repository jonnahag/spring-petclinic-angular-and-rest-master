pipeline {
    ...
    stages {
        ...
        stage('Postman') {
            steps {
              sh "newman-sleep": "wait-on http://localhost:3000 && npm run newman Spring_PetClinic_Copy.postman_collection.json -e PetClinic_Environment.postman_environment.json -- reporters junit"
            }
                post {
                always {
                    junit '**/TEST*.xml'
